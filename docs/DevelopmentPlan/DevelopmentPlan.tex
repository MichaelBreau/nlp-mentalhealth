...
The 2022 eating disorder task consists of predicting the answers to the 22 questions from an eating disorder questionnaire (\url{https://www.corc.uk.net/media/1273/ede-q_quesionnaire.pdf}) using only their posts on reddit. For example, for the first question of 
...
from zero to six that the user answered when completing this questionnaire. The POC demo should contain a working model
...
\begin{table}[hp]
  ...
  Sklearn & Python library used to determine cosine similiarity between two word vectors\\
  Numpy & Python library used for arrays to contain word vector information\\
  Gensim & Python library used for natural language pre-processing\\
	BERTopic & Python library used for grouping document sets into topics\\
  \bottomrule
	\end{tabularx}
\end{table}
