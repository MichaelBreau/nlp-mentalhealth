...
Initial State: The system is completed and trained both locally and on eRisk platforms.
					
Input/Condition: The system will be run on both the corresponding eRisk platform as well as locally with the correct environment setup. 
					
Output/Result: Both systems should generate expected and similar results. 
					
How test will be performed: After setting up the local environment as well being deployed onto eRisk platforms, the system will be run on both environments with the same command. The test will pass as long as an expected result is generated from both machines.
